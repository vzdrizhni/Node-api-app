...
exports.getPosts = async (req, res, next) => {
    ...
    let totalItems;
    try {
        totalItems = await Post.find().populate('creator').sort({createdAt: -1}).countDocuments();
  ...
  post
    ...
    .then(result => {
      ...
      .then(user => {
        ...
        res
            .status(201)
            .json({message: 'Post created', post: post, creator: {_id: creator._id, name: creator.name}})
      })
    .catch(err => {
      if (!err.statusCode) {
        err.statusCode = 500;
